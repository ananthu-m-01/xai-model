"""
Enhanced Multi-Modal Model WITHOUT sklearn dependencies
Resolves numpy 2.2.6 compatibility issues
"""
...
class AdvancedConfig:
    ...
    WEIGHT_DECAY = 1e-2
    PATIENCE = 15
    NUM_AUGMENTATIONS = 8  # Reduced for memory
    BATCH_SIZE = 6  # Reduced for 8GB GPU
    VAL_BATCH_SIZE = 6
    TEST_BATCH_SIZE = 6
    EPOCHS = 50
    LEARNING_RATE = 2e-5
    HIDDEN_DIM = 512
    NUM_HEADS = 8  # Reduced for memory
    NUM_LAYERS = 3  # Reduced for memory
    ...
    CHOSEN_MODEL = 'biomedbert'  # Medical specialist for best results
    ...
    # Default dimensions for brain features
    EEG_DIM = 16
    FMRI_DIM = 32
    
    # Transformer model name (same as chosen model)
    TRANSFORMER_MODEL = CHOSEN_MODEL

# Manual train/test split to avoid sklearn
def manual_train_test_split(data, labels, test_size=0.2, random_state=42):
    ...
    for label in unique_labels:
        label_indices = [i for i, l in enumerate(labels_array) if l == label]
...
# Manual evaluation functions to avoid sklearn
...
class AdvancedBrainDataset(Dataset):
    ...
    def _create_augmented_dataset(self):
        ...
        for i in range(self.num_augmentations):
            ...
            # Ensure we append explicit FloatTensor instances to satisfy static type expectations
            augmented_eeg.append(torch.FloatTensor(eeg_aug.detach().cpu().numpy()))
            augmented_fmri.append(torch.FloatTensor(fmri_aug.detach().cpu().numpy()))
            augmented_labels.append(self.original_labels)
        
        return (torch.cat([eeg.to(torch.float32) for eeg in augmented_eeg], dim=0),
                torch.cat([fmri.to(torch.float32) for fmri in augmented_fmri], dim=0),
                torch.cat([label.to(torch.long) for label in augmented_labels], dim=0))
...
class AdvancedEncoder(nn.Module):
    """Advanced encoder with attention layers."""
    ...
    def __init__(self, input_dim: int, hidden_dim: int, num_layers: int = 3, num_heads: int = 8, dropout: float = 0.3):
        ...
        # Transformer-like encoder layers (use proper nn.Module containers so static type checkers can resolve attributes)
        self.encoder_layers = nn.ModuleList()
        for _ in range(num_layers):
            layer = nn.Module()
            layer.attention = CrossModalAttention(hidden_dim, num_heads, dropout)
            ...
            layer.norm1 = nn.LayerNorm(hidden_dim)
            layer.norm2 = nn.LayerNorm(hidden_dim)
...
class AdvancedMultiModalNet(nn.Module):
    ...
    def _init_text_encoder(self):
        ...
        try:
            ...
            # Load appropriate model; some HF versions may return (model, config) tuples so unpack if needed
            if model_type == 'bert':
                model_obj = AutoModel.from_pretrained(model_path)
            elif model_type == 'roberta':
                model_obj = RobertaModel.from_pretrained(model_path)
            elif model_type == 'deberta':
                model_obj = AutoModel.from_pretrained(model_path)
            else:
                model_obj = AutoModel.from_pretrained(model_path)
            
            # If a tuple was returned (e.g., (model, config)), take the model element
            if isinstance(model_obj, tuple):
                model_obj = model_obj[0]
            
            self.text_encoder = model_obj
            # Also load and expose the tokenizer on the model for convenience
            try:
                self.tokenizer = AutoTokenizer.from_pretrained(model_path)
            except Exception:
                # Fallback tokenizer
            ...
            model_obj = DistilBertModel.from_pretrained('distilbert-base-uncased')
            if isinstance(model_obj, tuple):
                model_obj = model_obj[0]
            self.text_encoder = model_obj
...
def evaluate_model_advanced(model, data_loader):
    ...
    metrics = {
        ...
        'true_labels': true_labels,
        'classification_report': manual_classification_report(true_labels, predictions),
        'confusion_matrix': manual_confusion_matrix(true_labels, predictions)
    }
    
    if len(np.unique(true_labels)) > 1:
        metrics['auc'] = manual_roc_auc(true_labels, probabilities)
    else:
        metrics['auc'] = 0.0
    
    return metrics

def train_optimized(model, train_loader, val_loader, config, device):
    """
    Optimized training loop with advanced techniques:
    ...
    criterion = FocalLoss(alpha=config.FOCAL_ALPHA, gamma=config.FOCAL_GAMMA) if config.USE_FOCAL_LOSS else torch.nn.CrossEntropyLoss()
    for epoch in range(config.EPOCHS):
        model.train()
        running_loss = 0.0
        optimizer.zero_grad()
        for batch in train_loader:
            eeg, fmri, text, labels = batch
            eeg = eeg.to(device)
            fmri = fmri.to(device)
            labels = labels.to(device)
            text = {k: v.to(device) for k, v in text.items()}
            with autocast():
                outputs = model(eeg, fmri, text)
                loss = criterion(outputs, labels)
            scaler.scale(loss).backward()
            # Gradient clipping
            scaler.unscale_(optimizer)
...
def main_advanced():
    try:
        ...
        random.seed(AdvancedConfig.SEED)
        ...
        # Realistic EEG simulation (frequency bands)
        X_eeg = np.random.randn(n_samples, 16).astype(np.float32)
        
        # Realistic fMRI simulation (brain regions)
        X_fmri = np.random.randn(n_samples, 32).astype(np.float32)
        
        # Add meaningful signal based on labels
        ...
        train_val_idx, test_idx = manual_train_test_split(
            indices, labels, test_size=0.2, random_state=AdvancedConfig.SEED
        )
        
        train_idx, val_idx = manual_train_test_split(
            train_val_idx, labels[train_val_idx], test_size=0.2, random_state=AdvancedConfig.SEED
        )
        ...
        epoch = -1  # Initialize epoch to avoid unbound variable error
        ...
        for epoch in epoch_bar:
            ...
            if val_metrics['auc'] > best_val_auc:
                ...
                # Save only essential data to avoid pickle issues
                try:
                    torch.save(best_model_state, f'best_model_{AdvancedConfig.CHOSEN_MODEL}.pth')
                    ...
                    with open(f'best_model_{AdvancedConfig.CHOSEN_MODEL}_config.json', 'w') as f:
                        json.dump(config_save, f, indent=2)
                        
                except Exception as save_error:
                    logger.warning(f"Save error: {save_error}, continuing training...")
        ...
        # Safely determine number of epochs actually run (handle case where loop may not have executed)
        ...
        results = {
            ...
            'training_config': {
                ...
                'batch_size': int(AdvancedConfig.BATCH_SIZE),
                'learning_rate': float(AdvancedConfig.LEARNING_RATE),
                'hidden_dim': int(AdvancedConfig.HIDDEN_DIM),
                'num_heads': int(AdvancedConfig.NUM_HEADS),
                'num_layers': int(AdvancedConfig.NUM_LAYERS),
                'dropout': float(AdvancedConfig.DROPOUT),
                'use_mixup': bool(AdvancedConfig.USE_MIXUP),
                'use_focal_loss': bool(AdvancedConfig.USE_FOCAL_LOSS)
            },
                'use_mixup': bool(AdvancedConfig.USE_MIXUP),
                'use_focal_loss': bool(AdvancedConfig.USE_FOCAL_LOSS)
            },
            'test_metrics': {
                ...
                'confusion_matrix': [[int(cell) for cell in row] for row in test_metrics['confusion_matrix']]
            },
        ...
        # Save results (convert numpy/torch types to native Python types first)
...
def explain_prediction_gpt_style(model, eeg, fmri, text, device, feature_names=None, patient_info: Optional[dict] = None, use_generator: bool = False, generator_name: str = 'microsoft/BioGPT'):
    """
    Generate a GPT-style explanation for a single prediction.
    Returns a human-readable string explaining the model's decision.
    """
    import torch
    model.eval()
    with torch.no_grad():
        eeg = torch.as_tensor(eeg, dtype=torch.float32).unsqueeze(0).to(device)
        fmri = torch.as_tensor(fmri, dtype=torch.float32).unsqueeze(0).to(device)

        # Normalize text inputs: accept raw dict of strings, numpy arrays, or torch tensors
        text_input = {}
        for k, v in text.items():
            if isinstance(v, str):
                # tokenize string using model tokenizer if available
                tokenizer = getattr(model, 'tokenizer', None)
                if tokenizer is None:
                    raise RuntimeError('Tokenizer not available on model; please provide tokenized input')
                toks = tokenizer(v, return_tensors='pt', truncation=True, padding='max_length', max_length=AdvancedConfig.MAX_TEXT_LENGTH)
                for tk, tv in toks.items():
                    text_input[tk] = tv.to(device)
                break
            elif isinstance(v, torch.Tensor):
                t = v
                if t.dim() == 1:
                    t = t.unsqueeze(0)
                text_input[k] = t.to(device)
        ...
        # If tokenizer returned multiple keys above, ensure keys are correct for model
        if 'input_ids' in text_input and 'attention_mask' in text_input:
            model_input = {k: text_input[k] for k in ('input_ids', 'attention_mask') if k in text_input}
        else:
            model_input = text_input

        output = model(eeg, fmri, model_input)
        probs = torch.softmax(output, dim=1).cpu().numpy()[0]
        pred = int(probs.argmax())
    # Feature importance (simple: use absolute values)
    eeg_importance = abs(eeg.cpu().numpy()[0])
    fmri_importance = abs(fmri.cpu().numpy()[0])
    top_eeg_idx = int(eeg_importance.argmax())
    top_fmri_idx = int(fmri_importance.argmax())
    # Compose explanation
    explanation = f"The model predicts {'High Risk' if pred == 1 else 'Low Risk'} with confidence {probs[pred]:.2f}.\n"
    explanation += f"Most influential EEG feature: {feature_names['eeg'][top_eeg_idx] if feature_names and 'eeg' in feature_names else f'EEG_{top_eeg_idx}'} (value: {eeg_importance[top_eeg_idx]:.2f})\n"
    explanation += f"Most influential fMRI feature: {feature_names['fmri'][top_fmri_idx] if feature_names and 'fmri' in feature_names else f'fMRI_{top_fmri_idx}'} (value: {fmri_importance[top_fmri_idx]:.2f})\n"
    explanation += f"Text context: "
    # Show raw text if provided, otherwise indicate tokens
    for k, v in text.items():
        try:
            if isinstance(v, str):
                explanation += f"{k}: {v} | "
            elif isinstance(v, (list, tuple, np.ndarray)):
                explanation += f"{k}: tokens({len(v)}) | "
            elif hasattr(v, 'shape'):
                explanation += f"{k}: tensor{tuple(v.shape)} | "
            else:
                explanation += f"{k}: {str(v)[:60]} | "
        except Exception:
            explanation += f"{k}: <unavailable> | "
    explanation += "\n"
    explanation += "This decision is based on medical history, cognitive scores, and neural features."
    # Build an improved clinician-focused prompt
    patient_lines = []
    if isinstance(patient_info, dict):
        pid = patient_info.get('participant_id') or patient_info.get('participant') or patient_info.get('id')
        if pid:
            patient_lines.append(f"Patient ID: {pid}")
        # include age/sex if present
        if 'age' in patient_info:
            patient_lines.append(f"Age: {patient_info.get('age')}")
        if 'sex' in patient_info:
            patient_lines.append(f"Sex: {patient_info.get('sex')}")

    evidence_lines = [f"Predicted risk: {'High' if pred==1 else 'Low'} (confidence {probs[pred]:.2f})"]
    evidence_lines.append(f"Top EEG feature: {feature_names.get('eeg')[top_eeg_idx] if feature_names and 'eeg' in feature_names else f'EEG_{top_eeg_idx}'} (value {eeg_importance[top_eeg_idx]:.2f})")
    evidence_lines.append(f"Top fMRI feature: {feature_names.get('fmri')[top_fmri_idx] if feature_names and 'fmri' in feature_names else f'fMRI_{top_fmri_idx}'} (value {fmri_importance[top_fmri_idx]:.2f})")

    # Attempt to extract readable text context if tokenizer present
    text_snippets = []
    try:
        if hasattr(model, 'tokenizer') and hasattr(text, 'get'):
            # If text was tokenized earlier (text contains input_ids tensor), try to decode small slices
            if 'input_ids' in text:
                toks = text['input_ids']
                if isinstance(toks, torch.Tensor):
                    toks_cpu = toks.cpu()
                    # decode only first 128 tokens to avoid long output
                    decoded = model.tokenizer.decode(toks_cpu[0][:128], skip_special_tokens=True)
                    text_snippets.append(decoded)
        else:
            # fallback: join any string fields in a patient_info-like dict
    ...
    structured_explanation = "\n".join([
        "Evidence:",
        *(patient_lines if patient_lines else []),
        *evidence_lines,
        "Relevant text snippets:",
        *([s for s in text_snippets] if text_snippets else ["<no readable text available>"]) 
    ])

    # If generation requested, use a causal LM to paraphrase the structured explanation into a clinician note
    if use_generator:
        # Try biomedical generator first, then fall back to a small general model
        preferred_models = [generator_name, 'distilgpt2'] if generator_name != 'distilgpt2' else ['distilgpt2']
        last_exc = None
        for gen_name in preferred_models:
            try:
                # Load tokenizer and model; suppress verbose HF logs during generation
                ...
                # Build a stricter clinician-oriented prompt with explicit sections and safe handling of unknown feature names
                prompt = (
                    "You are a concise clinical assistant writing for a neurologist/geriatrician.\n"
                    "Task: From the structured evidence below, produce a short clinician note with THREE labeled sections exactly: \n"
                    "1) Key finding (one sentence)\n"
                    "2) Interpretation and clinical relevance (one to two sentences)\n"
                    "3) Recommended next steps (1-3 short bullet points, prioritized)\n\n"
                    "Guidelines:\n"
                    "- Use only the information present in the evidence; do not invent or assume additional clinical details.\n"
                    "- If a feature name is not clinically specific (e.g., 'EEG_5' or 'fMRI_30'), describe it neutrally as 'EEG feature #5 (derived metric)' or 'fMRI feature #30 (derived metric)' and link interpretation clearly to the evidence (e.g., 'elevated value' or 'reduced value').\n"
                    "- Be explicit about uncertainty (e.g., 'findings suggest', 'may reflect', 'consider') and avoid absolute claims.\n"
                    "- For recommended next steps, prioritize commonly accepted follow-ups for cognitive decline / dementia risk (neuropsychological testing, structural MRI with hippocampal volumetry, specialist referral, CSF or PET biomarkers) and include rationale.\n\n"
                    "Structured evidence:\n"
                    f"{structured_explanation}\n\n"
                    "Output (only the 3 labeled sections):\n1) Key finding:\n"
                )

                inputs = gen_tokenizer(prompt, return_tensors='pt', truncation=True, padding=True).to(device)
                with torch.no_grad():
                ...
                # `outputs[0]` may contain the input prompt followed by generated tokens.
                # Decode only the newly generated tokens (after the input length) to avoid repeating the prompt.
                gen_ids = outputs[0]
                try:
                    input_len = inputs['input_ids'].shape[1]
                    gen_only = gen_ids[input_len:]
                    if gen_only.numel() == 0:
                        # fallback to decoding whole sequence
                        generated = gen_tokenizer.decode(gen_ids, skip_special_tokens=True)
                    else:
                        generated = gen_tokenizer.decode(gen_only, skip_special_tokens=True)
                except Exception:
                    # conservative fallback
                    generated = gen_tokenizer.decode(gen_ids, skip_special_tokens=True)

                # Try to extract clinician note: look for the marker, else use generated text
                split_marker = 'Clinician note:'
                if split_marker in generated:
                    text_out = generated.split(split_marker, 1)[1].strip()
                else:
                    text_out = generated.strip()

                # Keep only first 3 sentences for conciseness
                import re
                ...
                continue
        logger.warning(f"All generators failed, falling back to structured explanation: {last_exc}")
        # Fall back to deterministic clinician note
        use_generator = False

    # Deterministic clinician-facing note (safe, no external LM needed)
    # Build readable feature names
    try:
        eeg_name = feature_names.get('eeg')[top_eeg_idx] if feature_names and 'eeg' in feature_names else f'EEG feature #{top_eeg_idx}'
    except Exception:
        eeg_name = f'EEG feature #{top_eeg_idx}'
    try:
        fmri_name = feature_names.get('fmri')[top_fmri_idx] if feature_names and 'fmri' in feature_names else f'fMRI feature #{top_fmri_idx}'
    except Exception:
        fmri_name = f'fMRI feature #{top_fmri_idx}'

    confidence_pct = probs[pred] * 100.0
    key_finding = f"Key finding: The model predicts {'HIGH' if pred==1 else 'LOW'} risk (confidence {confidence_pct:.1f}%). Top contributors in this sample were {eeg_name} (value {eeg_importance[top_eeg_idx]:.2f}) and {fmri_name} (value {fmri_importance[top_fmri_idx]:.2f})."
    interpretation = (
        "Interpretation: These signal-level findings may reflect neural changes associated with cognitive decline or increased dementia risk; "
        "they should be interpreted in the context of the patient's history and cognitive test results."
    )
    recommendations = (
        "Recommended next steps:\n"
        "- Perform formal neuropsychological testing to quantify cognitive domains affected.\n"
        "- Obtain structural MRI with attention to medial temporal/hippocampal atrophy or consider quantitative volumetry if available.\n"
        "- Discuss referral to neurology or memory clinic; consider CSF or PET biomarker testing if clinically indicated."
    )

    clinician_note = "1) " + key_finding + "\n\n2) " + interpretation + "\n\n3) " + recommendations
    return clinician_note
